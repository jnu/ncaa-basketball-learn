...
object Learn {
    ...
    def runInContext(sc: SparkContext, h2oContext: H2OContext): Unit = {
        ...
        // Load source file
        val sourceFile = "data/merged_data.csv"

        val dataFrame: DataFrame = new DataFrame(new File(sourceFile))
        val summaryTable: RDD[MergedGameData] = asRDD[MergedGameData](dataFrame)
